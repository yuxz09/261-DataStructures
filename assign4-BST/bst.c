...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    if (cur == 0)
    {
        ...
        newnode->val = val;
        newnode->right = 0; /* or SetHeight on new Node */
        return newnode;  /* No need to balance here! */
    }
    else if((*compare)(val, cur->val) == -1)//if  value < current node value
    {
...
//argument is tree, pass parameters are tree->root->left/right, it is node
...
/*
 ...
 HINT: this function returns cur if its left child is NOT NULL. Otherwise, //why this hint
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    if((*compare)(cur -> val, val) == 0)
    {
        ...
        else//cur -> right == 0
        {
            ...
            free(cur);
            return temp;
           // free(cur);/////why not free cur here?????????
        }
    }
